...
const DrawRotation = new NativeClass('Microsoft.Xna.Framework.Graphics', 'SpriteBatch')[
	...
	if (nearestNPCIndex !== -1) {
		...
		if (npc.active) {
			...
			if (distance > minDistance) {
				...
				const offset = nVector2(Math.cos(rotation) * offsetDistance, Math.sin(rotation) * offsetDistance);
				...
				DrawRotation(Main.spriteBatch, texPoint, screenArrowPos, null, Color.White, rotation, textureOrigin, scale, SpriteEffects.None, 0);
			}
		}
	}
});
